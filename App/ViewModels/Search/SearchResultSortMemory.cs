...
// (see FileExecutor's "__SHOW_MORE__" path, which creates a new SearchWindow unless the user enables
// single-instance mode), so a plain instance field would reset on every newly opened window even within
// the same session. A process-wide static is
