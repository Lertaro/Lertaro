...
internal static class DirMenuIcon
{
    ...
    public static IntPtr GetRootHBitmap()
    {
        lock (Sync)
        {
            if (_rootCached == IntPtr.Zero) _rootCached = Render(FloppyDiskPath, viewBoxSize: 24);
            return _rootCached;
        }
    }
    ...
    // Called from DirMenuProvider.ClearSession, once per popup session: no row from the just-closed
