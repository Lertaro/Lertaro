...
`http://`/`https://` から始まるアドレスを入力または貼り付けると、Lertaro がそれを直接開くことを提案します。
...
| `flow` | Flow プラグインブリッジ | 読み込まれたすべての Flow.Launcher プラグインとそのトリガーキーワードを一覧表示（スペースに続けてフィルター可能）。プラグイン設定は**設定 → プラグイン**で直接統一管理されます。 |
...
は、ブラウザで「lertaro github」の Google 検索を開きます。

## 翻訳

翻訳プラグインの既定キーワードは `tr` です。後にテキストを入力すると、Lertaro のインターフェイスで現在選択されている言語へ翻訳します:

```
tr hello
```

結果は非同期で表示されます。**Enter** を押すかクリックするとコピーできます。翻訳するテキストは Microsoft Translator に送信されるため、インターネット接続が必要です。翻訳先は App の現在の言語設定です。キーワードは **設定 → プラグイン → 翻訳 → 設定** で変更できます。
