...
- **監視フォルダー**：設定したローカルフォルダーをホストのインデックスサービスに登録します。初回および増分の検出は、SDK のインデックス済みディレクトリ列挙とデバウンスされたディレクトリ変更通知で行われます。各フォルダーはホストのローカルドライブインデックスまたはフォルダーインデックスの対象になっている必要があり、対象外の場合は列挙結果が空になります。`%USERPROFILE%` などの環境変数にも対応します。
