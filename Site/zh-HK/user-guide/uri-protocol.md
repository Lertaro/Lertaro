...
Lertaro 在首次運行時會自動在 Windows 系統中註冊自訂協議 **`lertaro://`**。無論是網頁超連結、桌面捷徑、自動化腳本還是第三方軟體，均可透過該協議直接喚起 Lertaro 的特定搜尋、直達設定頁面或發起跨裝置傳輸。

## 1. 協議機制與執行個體路由

- **開箱即用**：無需手動設定註冊表，Lertaro 啟動時會自動完成註冊與自癒校驗。
- **單一實例路由**：若 Lertaro 已經在後台運行，開啟 `lertaro://` 連結會直接喚醒當前運行中的前台執行個體，絕不會重複啟動多個程序；若 Lertaro 尚未啟動，系統會自動拉起主程式並立即執行連結指定的動作。
...
- **安全約束**：所有傳入的檔案路徑必須為本機已經真實存在的絕對路徑；帶有預填內容的連結僅會開啟 LocalSend 並進入裝置選擇介面，絕不會自動向任何裝置傳送資料。
...
- **無訊息容錯**：由於任何外部網頁或腳本均可嘗試觸發該協議，Lertaro 對所有傳入的 URI 實行嚴格的白名單校驗。若連結格式錯誤、拼寫有誤或指向不存在的路由，Lertaro 會直接安全忽略，僅記錄偵錯記錄，絕不會產生意外的破壞性操作或當機。
