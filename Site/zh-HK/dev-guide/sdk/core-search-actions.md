...
public interface ISearchResultAction : IPluginComponent
{
    ...
    string? Hotkey { get; }             // 預設快捷鍵（如 "Ctrl+Shift+C"）
...
public interface IDynamicActionProvider
{
    ...
    int? Priority => 0;                 // 在動作選單中的預設展示優先級
    IReadOnlyList<string>? Keywords { get; }
    IReadOnlyList<string>? Parameters { get; }
    bool IsVisibleInSearch(IReadOnlyList<ISearchResult> selection, SearchWindowType windowType);
    bool IsVisibleInMenu(IReadOnlyList<ISearchResult> selection, SearchWindowType windowType);
    void Init() { }                     // 程序生命週期內最多觸發一次的預熱初始化
...
- **`DynamicMenuItem`**：包含 `Text`、`CommandId`、`IsSeparator`、`HasSubMenu`、`SubMenuHandle`、`IsDisabled`、`IsActionable`、`IsContinuation`、`OnExecute`、`IsHeader` 和 `ShortcutHint`。純分類節點只負責展開子選單時應設定 `IsActionable = false`；真實資料夾節點可以保留預設值 `true`。`IsContinuation = true` 表示分頁子選單的延續游標，主機會自動載入下一頁而不會轉譯可見的「載入更多」列。`IsHeader` 會將項目轉譯為帶可選操作按鈕的分組標題列。
