...
- **Community Store & Package Management (`flow install` / `flow update` / `flow uninstall`)**: Type `flow install` or `flow install <plugin-name/keyword>` in the search bar to query the official Flow.Launcher community plugin repository. Press Enter on any result to automatically download, verify, unpack, and provision it under `FlowData\Plugins\`, while silently installing all runtime dependencies in the background. Type `flow update` to check for and install updates for your installed Flow plugins, or `flow uninstall <plugin-name>` to remove a plugin and clean up its directory. You can also manually drop third-party plugin folders into `<User Data Directory>\FlowData\Plugins\`.
