...
| `ExplorerPathService` | `GetLastActivePath()` —— 檔案總管視窗或檔案對話方塊最後顯示的那個資料夾，從來沒有過則為 `null`。它由宿主自己的視窗追蹤填入，跟的是**所有**應用程式的檔案對話方塊，而不只是 Lertaro 自己的介面，所以它的含義是「使用者最後真正在看的那個資料夾」——這是外掛自己算不出來的。方向和 [`IActivePathCollector`](./core-search-actions) 相反：那個是外掛**告訴**宿主某個第三方檔案管理員正在顯示什麼，這個是向宿主打聽。不保證仍然存在：它記的是使用者去過哪，而那個資料夾可能早已被刪掉或拔掉了。 |
...
| `UserDataService` | `GetUserDataDirectory()` —— 回傳目前使用者的專屬資料目錄（如安裝版的 `%LOCALAPPDATA%\Lertaro` 或便攜版的 `Data\Users\<hash>`），用於存放使用者私有設定與外掛資料；`GetSharedDataDirectory()` —— 回傳機器級全域共享資料目錄（如安裝版的 `%PROGRAMDATA%\Lertaro` 或便攜版的 `Data\Machine`），用於跨多使用者共享執行階段（如 Python / Node.js 嵌入環境）與全域公共快取。 |
