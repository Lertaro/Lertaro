...
| **Seleccionar elemento siguiente** | `Ctrl+N` o `↓` | Mueve el resaltado hacia abajo. En el Panel rápido se desplaza fluidamente entre grupos. Con el Panel de inicio rápido visible y la consulta vacía, `Ctrl+N` recorre la siguiente fuente y vuelve a la primera al llegar al final. |
| **Seleccionar elemento anterior** | `Ctrl+P` o `↑` | Mueve el resaltado hacia arriba. En el Panel rápido también se desplaza entre grupos. Con el Panel de inicio rápido visible y la consulta vacía, `Ctrl+P` recorre la fuente anterior y vuelve a la última al llegar al principio. |
